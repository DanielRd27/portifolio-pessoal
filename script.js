...
menuIcon.addEventListener('click', () => {
    ...
    // Bloquear scroll quando menu for aberto
...
// Função para alterar entre os temas claro e escuro
function toggleMode () {
...
function animateText () {
    if (isTyping) {
        if (index < text.length) {
            // Adiciona uma letra ao título
            titleElement.textContent = text.slice(0, index + 1);
            index++;
        } else {
            // Alterna para o modo de apagamento
            isTyping = false;
        }
    } else {
        if (index > 1) {
            // Remove uma letra do título
            titleElement.textContent = text.slice(0, index - 1);
            index--;
        } else {
            // Alterna para o modo de digitação
            isTyping = true;
    ...
    // Define um intervalo para a próxima animação
...
// Observa para animar as seções ao rolar a página
...
// Adiciona um evento de clique ai botão de voltar ao topo
...
function showSlide(slideIndex) {
    ...
    // Ajusta o índice de slide para garantir que ele esteja dentro dos limites
...
function updateSlidePosition() {
    const slideWidht = slides[0].offsetWidth;
...
// Pausa a transição ao passar o mouse sobre o carrossel
...
// Seleciona o formulário de contato e a mensagem de agardecimento 
...
contactForm.addEventListener('submit', (e) => {
    ...
    .then(response => {
        if(response.ok) {
            setTimeout(() => window.location.reload(),2000) // Recarrega a páginaapós 2 segundos
        } else {
            alert('Erro ao enviar fomrluário. Tente novamente');
        }
    })
    .catch(() =>alert('Erro na conexão. Tente novamente'));
});

// ====== ANIMAÇÃO SA SEÇÃO "SOBRE MIM" ======
...
function checkAboutVisibility() {
    ...
    window.removeEventListener('scroll', checkAboutVisibility); // Remove o listaner após a animação.
    }
}

// Adiciona um listaner para o evento de scroll
